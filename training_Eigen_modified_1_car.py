...
MODEL_NAME = "Eigen_modified_1_car"
...
def main():
    ...
    coarse5 = tf.layers.conv2d(coarse3, 256, 3, activation=tf.nn.relu, padding='valid')
    ...
    loss1 = lossFunc(coarse7_output, y_, m_)
    ...
    with tf.Session() as sess:
        ...
        for i in range(it_count):
            ...
            train_step1.run(feed_dict={x: im, y_: dp, m_:ms})
            printProgress(i+1, it_count)
            if i % 50 == 0:
                [imt, dpt, mst] = rd.getNextBatchTesting(10)
                tmpTL = loss1.eval(feed_dict={x: imt, y_: dpt, m_:mst, keep_prob: 1})
                tmpLoss = loss1.eval(feed_dict={x: im, y_: dp, m_:ms, keep_prob: 1})
